...
if (!$movieImg)
    throw new Error('$movieImg not found.');
if (!$movieTitle)
    throw new Error('$movieTitle not found.');
if (!$releaseTitle)
    throw new Error('$releaseTitle not found.');
if (!$summary)
    throw new Error('$summary not found.');
if (!$movieRankingWrapper)
    throw new Error('$movieRankingWrapper not found.');
if (!$movieRanking)
    throw new Error('$movieRanking not found.');
if (!$votesNumber)
    throw new Error('$votesNumber not found.');
if (!$genrePillsDiv)
    throw new Error('$genrePillsDiv not found.');
if (!$exitToggle)
    throw new Error('$exitToggle not found.');
if (!$openToggle)
    throw new Error('$openToggle not found.');
if (!$sidebar)
...
if (!$detailsIcon)
    throw new Error('$detailsIcon not found.');
if (!$noFavorites)
    throw new Error('$noFavorites not found.');
if (!$loadingDiv)
    throw new Error('$loadingDiv not found.');
let moviesArr = [];
const genreMap = {
    28: 'Action',
    12: 'Adventure',
...
function renderCard(movieData) {
    ...
    $cardImage.setAttribute('src', 'http://image.tmdb.org/t/p/w500/' + movieData.poster_path);
    ...
    if (!iconId)
        throw new Error('iconId not found.');
...
function renderMovieDetails(movieData) {
    $movieImg.setAttribute('src', 'http://image.tmdb.org/t/p/w500/' + movieData.poster_path);
...
$row.addEventListener('click', (event) => {
    ...
    if (!$eventTarget.matches('a') && !$eventTarget.matches('i'))
        return;
    const $allHomeIcons = document.querySelectorAll('.home-icon');
    if (!$allHomeIcons)
        throw new Error('$allHomeIcons not found.');
    ...
    if ($eventTarget.matches('a')) {
        ...
        $allHomeIcons.forEach((icon) => {
            ...
            if ($iconId === cardId && !icon.classList.contains('fa-solid')) {
                ...
                if (!$allCardElements)
                    throw new Error('$allCardElements not found.');
